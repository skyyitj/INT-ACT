...
def create_server_config(model_path, port=5000):
    """创建服务器配置文件"""
    print("\n📝 创建服务器配置...")
...
def create_client_config(model_path, port=5000, n_episodes=20, n_videos=5):
    """创建客户端配置文件"""
...
def main():
    ...
    parser.add_argument("--episodes", type=int, default=3, help="每个任务的测试轮数")
    parser.add_argument("--videos", type=int, default=2, help="录制视频的数量")
    ...
    # 3. 创建配置
    server_config = create_server_config(model_path, args.port)
    client_config = create_client_config(model_path, args.port, args.episodes, args.videos)

"""
From Allen's Open-pi-zero: https://github.com/allenzren/open-pi-zero 
...
def chunk_act_obs_also_future(
    traj: dict,
    window_size: int = 1,
    action_horizon: int = 1,
    max_action_future: Optional[int] = None,
) -> dict:
    """Chunks actions and observations.
    "observation" keys are given a new history axis, making them of shape [traj_len, window_size, ...],
    containing the observation history at each timestep from `t - window_size + 1` to `t`.
    ...
    traj["future_action"] = tf.identity(traj["action"])
    if max_action_future is None:
        max_action_future = traj_len
        
    if len(traj["future_action"].shape) == 2:
        # actions are not pre-chunked
        future_action_chunk_indices = tf.range(traj_len)[:, None] + tf.range(
            max_action_future
        )# [traj_len, max_action_future]
        # repeat the last action at the end of the trajectory rather than going out of bounds
        future_action_chunk_indices = tf.minimum(future_action_chunk_indices, traj_len - 1)
        # gather
        traj["future_action"] = tf.gather(
            traj["future_action"], future_action_chunk_indices
        )
    else:
        # actions are pre-chunked, so we don't add a new axis
        if traj["future_action"].shape[1] < traj_len:
            raise ValueError(
                f"all future action horizon ({traj_len}) is greater than the pre-chunked action dimension ({traj['future_action'].shape[1]})"
            )
        traj["future_action"] = traj["future_action"][:, :traj_len]
    
    # then, add the history axis to actions
    traj["future_action"] = tf.gather(
        traj["future_action"], history_indices
    )  # [traj_len, window_size, max_action_future, action_dim]
    ########################################################################
    # first, chunk actions into `action_horizon` current + future actions
    if len(traj["action"].shape) == 2:
        # actions are not pre-chunked
    ...
    # tf.print("traj len", traj_len)
    # tf.print("action shape", tf.shape(traj["action"]))
    # tf.print("goal_timestep:", goal_timestep)
    # tf.print("goal_timestep size", tf.shape(goal_timestep))
    ...
    # future action pad mask
    again_future_action_chunk_indices = tf.range(traj_len)[:, None] + tf.range(
        max_action_future
    )# [traj_len, max_action_future]
    # extend to history dim
    again_future_action_chunk_indices = tf.gather(
        again_future_action_chunk_indices, history_indices
    ) # [traj_len, window_size, max_action_future]
    # tf.print("again_future_action_chunk_indicies", tf.shape(again_future_action_chunk_indices))
    
    traj["future_action_pad_mask"] = tf.logical_and(
        # [traj_len, 1, 1, action_dim]
        (
            traj["action_pad_mask"][:, None, None, :]
            if len(traj["action_pad_mask"].shape) == 2
        ...
        # if reach goal timestep
        # again_future_action_chunk_indices [traj_len, max_action_future]
        # goal_timestep [traj_len]
        again_future_action_chunk_indices[:, :, :, None] <= goal_timestep[:, None, None, None] # size
    )
        
    ########################################################################
    # broadcast "action_pad_mask" to the new chunked shape, and mark actions past the goal timestep as padding
    # * 1 for actual value, 0 for padding
    traj["action_pad_mask"] = tf.logical_and(
        # [traj_len, 1, 1, action_dim]
        (
            traj["action_pad_mask"][:, None, None, :]
    ...
    # add trajectory length to the traj
    traj["traj_len"] = tf.fill([traj_len], traj_len)
    return traj

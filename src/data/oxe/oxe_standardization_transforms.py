...
import tensorflow_graphics.geometry.transformation as tfg  # it's weird, but we have to import this instead of simply tf graphics to use geometry
...
def taco_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trajectory["observation"]["proprio"] = tf.concat(
        (
            ...
            # trajectory["observation"]["robot_obs"][:, 7:8], this seems wrong according to documentation. Note gripper action but 0th joint state
            trajectory["observation"]["robot_obs"][:, -1:],
        ),
        axis=-1,
    )
...
###### added by Irving ###
def taco_libero_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    '''
    The assumption is that taco_play uses yaw-pitch-roll as specified in https://www.tensorflow.org/datasets/catalog/taco_play
    I am additionally assume that it's using szyx as commonly used in robotics.
    So, first x rotation, then y rotation then z rotation.
    Tensorflow graphics also first x, then y, then z rotation.
    '''
    trajectory["action"] = trajectory["action"]["rel_actions_world"]
    ypr_action = trajectory["action"][:, 3:6] # action with rotation in euler angles yaw pitch roll
    # convert to roll pitch yaw
    indices = tf.constant([2, 1, 0])
    rpy_action = tf.gather(ypr_action, indices, axis=1)
    # convert to axis-angle
    axis, angle = tfg.axis_angle.from_euler(rpy_action)
    axis_angle_action = axis * angle
    # clip gripper action, +1 = open, 0 = close
    gripper_action = tf.clip_by_value(trajectory["action"][:, -1:], 0, 1)
    # make it the same as libero where +1 = close, -1 = open
    gripper_action = 1-2*gripper_action
    trajectory["action"] = tf.concat(
        (
            trajectory["action"][:, :3],
            axis_angle_action,
    ...
    ypr_state = trajectory["observation"]["robot_obs"][:, 3:6]
    # convert to roll pitch yaw
    indices = tf.constant([2, 1, 0])
    rpy_state = tf.gather(ypr_state, indices, axis=1)
    # convert to axis-angle
    axis, angle = tfg.axis_angle.from_euler(rpy_state)
    axis_angle_state = axis * angle

    # 1 = open, -1 = close
    gripper_state=trajectory["observation"]["robot_obs"][:, -1:]
    # flip the gripper state. 1 = close, -1 = open
    gripper_state = -1*gripper_state

    # print(trajectory["observation"]["robot_obs"][:, :3].shape)
    # print(axis_angle_state.shape)
    # print(gripper_state.shape)
    trajectory["observation"]["proprio"] = tf.concat(
        (
            trajectory["observation"]["robot_obs"][:, :3],
            axis_angle_state,
            gripper_state,
        ),
        axis=-1,
...
def libero_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # since no mix training. I will not modify the gripper action
    ...
    gripper_state_2d = trajectory["observation"]["state"][:, -2:]  # 2D gripper state
    # Compute the minimum of the absolute values along the last dimension:
    min_abs_val = tf.reduce_min(tf.abs(gripper_state_2d), axis=1)
    # Compare with the threshold: 1 if less than threshold, else -1
    gripper_state_1d = tf.where(min_abs_val < 0.015, tf.ones_like(min_abs_val), -tf.ones_like(min_abs_val))
    # Expand dims to get shape [batch_size, 1]
    gripper_state_1d = tf.expand_dims(gripper_state_1d, axis=-1)
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "fractal_euler": rt1_dataset_transform,
    ...
    ### added by Irving ###
    "taco_play_libero": taco_libero_dataset_transform,
